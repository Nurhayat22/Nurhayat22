# Salam, mən Nurhayat 👋

### Junior Data Analyst

Data təmizləmə, analiz və vizuallaşdırma vasitəsilə biznes qərarlarına dəyər qatmağa çalışıram.

---

### 🧰 Texniki bacarıqlar

| Sahə | Alətlər |
|---|---|
| Data sorğulama | SQL |
| Data analizi | Python (Pandas) |
| Vizuallaşdırma | Power BI |
| Hesabat & modelləşdirmə | Excel |

### 📊 Hazırda
- Real dataset-lər üzərində uçdan-uca analiz layihələri hazırlayıram
- SQL və Python bacarıqlarımı dərinləşdirirəm

### 📌 Seçilmiş layihələr

- [SQL Retail Sales Analysis](https://github.com/Nurhayat22/sql-retail-sales-analysis) — Superstore satış datası üzərində SQL sorğuları (JOIN, subquery, window function və s.)

### 📫 Əlaqə
LinkedIn: [linkedin.com/in/nurhayat-asgarova](https://www.linkedin.com/in/nurhayat-asgarova/)
Email: nuraskerovva@gmail.com

---
